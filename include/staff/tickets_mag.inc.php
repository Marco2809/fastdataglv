...
$total=db_count("SELECT count(CASE WHEN status.name='In carico al magazzino' OR status.name='In attesa parti' THEN 1 END) $qfrom $sjoin $qwhere");
/*
...
$query="$qselect $qfrom $qwhere "."AND (ticket.status_id=11 OR ticket.status_id=19)"." ORDER BY $order_by $order LIMIT ".$pageNav->getStart().",".$pageNav->getLimit();
//echo $query;
//$hash = md5($query);
     ...
     <tbody style="border-radius:25px;">
        ...
        if($res && ($num=count($results))):
            ...
            foreach ($results as $row) {
                ...
                if($row['status']=="In carico al magazzino" || $row['status']=="In attesa parti") {
                ?> 
