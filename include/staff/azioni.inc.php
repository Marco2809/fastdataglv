...
$template_scontrino="<div>'+input_count_ini+'&nbsp;<input type=\"text\" id=\"scontrino_'+input_count+'\" name=\"input_field_scontrino['+input_count+'][]\" value=\"\" placeholder=\"Ticket o ordine\"/><a href=\"javascript:void(0);\" class=\"remove_input_button_scontrino\" title=\"Remove field\"></a><input type=\"text\" name=\"input_field_scontrino['+input_count+'][]\" value=\"\" placeholder=\"Ora Scontrino\"/><a href=\"javascript:void(0);\" class=\"remove_input_button_scontrino\" title=\"Remove field\"><img src=\"../include/staff/img/remove.png\"/></a><span id=\"result_scontrino_'+input_count+'\"></span></div>";
...
<select name='status_id'>
          ...
          <option value="scontrino">Data Scontrino</option>
</select><br><br>
...
01&nbsp;<input type="text" id="ripresa_0" name="input_field3[0][]" value="" placeholder="Ticket o ordine"/><a href="javascript:void(0);" class="add_input_button3" title="Add field"></a><input type="text" name="input_field3[0][]" value="" placeholder="Motivo ripresa"/><a href="javascript:void(0);" class="add_input_button3" title="Add field"><img src="../include/staff/img/add.png"/></a><span id="result_ripresa_0"></span>
    ...
    <div id="scontrino" style="display:none">
        <div class="field_wrapper_scontrino">
            <div>
                01 <input type="text" id="scontrino_0" name="input_field_scontrino[0][]" value="" placeholder="Ticket o ordine"/><a href="javascript:void(0);" class="add_input_button_scontrino" title="Add field"></a><input type="text" name="input_field_scontrino[0][]" value="" placeholder="Ora Scontrino"/><a href="javascript:void(0);" class="add_input_button_scontrino" title="Add field"><img src="../include/staff/img/add.png"/></a><span id="result_scontrino_0"></span>
            </div>
        </div>
    </div>
...
$(document).ready(function () {
    $('#azioni select[name="status_id"]').change(function () {
        if ($('#azioni select[name="status_id"] option:selected').val() ==2) {
            ...
            $('#ripreso').hide();
            $('#scontrino').hide();
        } else if ($('#azioni select[name="status_id"] option:selected').val() =='scontrino') {
            $('#scontrino').show();
            $('#chiusura').hide();
            $('#recesso').hide();
            $('#attesa').hide();
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        var input_count = 1;
        var input_count_ini = input_count+1;
        var add_input_button = $('.add_input_button_scontrino');
        var field_wrapper = $('.field_wrapper_scontrino');

// Add button dynamically
        $(add_input_button).click(function(){
            if(input_count < max_fields){
                if(input_count_ini<10) input_count_ini = '0'+input_count_ini;
                $(field_wrapper).append('<?php echo $template_scontrino;?>');
                var script = document.createElement("script");
                script.innerHTML = "$('#scontrino_"+input_count+"').on('change paste keyup',function(){var num = encodeURIComponent($('#scontrino_"+input_count+"').val());$('#save').prop('disabled', false);});";
                $(field_wrapper).append(script);
                input_count_ini++;
                input_count++;
            }
...
// Remove dynamically added button
        $(field_wrapper).on('click', '.remove_input_button_scontrino', function(e){
            e.preventDefault();
            $(this).parent('div').remove();
            if($('[id^="result_scontrino_"]').text().includes('Ticket non esistente')) $('#save').prop( "disabled", true);
            else $('#save').prop( "disabled", false);
            input_count--;
            input_count_ini--;
            if(c==1) c--;
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#scontrino_0').on('change paste keyup',function(){
//alert('test');
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#result_scontrino_0').html(response);
                    //if(response=='Ticket non esistente') $('#save').prop( "disabled", true);
                    //else if(response=='OK') $('#save').prop( "disabled", false);'[id^="content_"]'
                    //alert($('[id^="result_chiusura_"]').text());
                    if($('[id^="result_scontrino_"]').text().includes('Ticket non esistente')) $('#save').prop( "disabled", true);
