...
$qwhere.=' AND ticket.topic_id IN (15,16,36,38,39) ';
...
//echo "SELECT count(CASE WHEN status.name='Transferred' OR status.name='Planning' OR status.name='Riparato LAB' OR status.name='Lavorato magazzino' OR status.name='Non risolto' OR status.name='Da validare PLN' THEN 1 END) $qfrom $sjoin $qwhere";

$total=db_count("SELECT count(CASE WHEN status.name='Transferred' OR status.name='Planning' OR status.name='Riparato LAB' OR status.name='Lavorato magazzino' OR status.name='Non risolto' OR status.name='Ripreso in carico' THEN 1 END) $qfrom $sjoin $qwhere");
...
$query_exp="$qselect $qfrom $qwhere AND ticket.status_id!=21 ORDER BY $order_by $order LIMIT ".$pageExp->getStart().",".$pageExp->getLimit();
...
<?php if($thisstaff->getDeptId()==5 || $thisstaff->getDeptId()==6 ) {
  $query_aperti=substr($query_exp, 0, strpos( $query_exp, ' WHERE'));
	$query_aperti .= " WHERE ticket.status_id IN (1,7,22) AND ticket.topic_id IN (15,16,36,38,39) ORDER BY ticket.created DESC LIMIT 0,100 ";
...
<center>
 <table width="100%" border="0" cellspacing="20px" cellpadding="10px" style="border-collapse: separate; border-spacing: 0px 10px; margin-top:-10px; margin-bottom:10px; background:transparent; ">
  ...
  $query_attesa=substr($query_exp, 0, strpos( $query_exp, ' WHERE'));
	$query_attesa .= " WHERE ticket.status_id IN (21) AND ticket.topic_id IN (15,16,36,38,39) ORDER BY ticket.created DESC LIMIT 0,100 ";
 ...
 <table width="100%" border="0" cellspacing="20px" cellpadding="10px" style="border-collapse: separate; border-spacing: 0px 10px; margin-top:-10px; margin-bottom:10px; background:transparent; ">
  ...
  $query_assegnato=substr($query_exp, 0, strpos( $query_exp, ' WHERE'));
	$query_assegnato .= " WHERE ticket.status_id IN (23) AND ticket.topic_id IN (15,16,36,38,39) ORDER BY ticket.created DESC LIMIT 0,100 ";
 ...
 <table width="100%" border="0" cellspacing="20px" cellpadding="10px" style="border-collapse: separate; border-spacing: 0px 10px; margin-top:-10px; margin-bottom:10px; background:transparent; ">
  ...
  $query_recesso=substr($query_exp, 0, strpos( $query_exp, ' WHERE'));
	$query_recesso .= " WHERE ticket.status_id IN (8) AND ticket.topic_id IN (15,16,36,38,39) ORDER BY ticket.created DESC LIMIT 0,100 ";
               ...
               if ($row['imac']==1) { //backend per me
                ...
                <td width="3%" nowrap >
                        <a <?php echo $staff_sort; ?> href="tickets.php?sort=staff&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                            title="<?php echo sprintf(__('Sort by %s %s'), __("Closing Agent's Name"), __($negorder)); ?>"><strong style="font-weight:bold;"><?php echo __('Closed By').':'; ?></strong></a><br><span style="font-size:80%; font-family:play;"><?php echo $row['pln_alpha']==1?'Poste s.p.a.':$lc;?></span></td>
                <?php }?>
                <?php if(($thisstaff->getId()==77 || $thisstaff->getId()==78 || $thisstaff->getId()==79) AND strtolower($row['status'])=='risolto' AND strtolower($_REQUEST['a'])=="search") {?>
            ...
            <select id="assignee" name="assignee">
                ...
                if(($users=Staff::getStaffMembers())) {
                    ...
                    foreach($users as $id => $name) {
                        ...
                        if (in_array($id,array($thisstaff->getId(),1,78,14,3,5,63,79)))
                            continue;
                        $k="s$id";
                        echo sprintf('<option value="%s">%s</option>', $k, $name);
                    }
    ...
    /*
        ...
        <fieldset class="owner span6">
            ...
            <select id="staffId" name="staffId">
                ...
                if(($users=Staff::getStaffMembers())) {
                    foreach($users as $id => $name){
                        if (in_array($id,array($thisstaff->getId(),1,14,5,63)))
                            continue;
                        echo sprintf('<option value="%d">%s</option>', $id, $name);
                    }
                }
