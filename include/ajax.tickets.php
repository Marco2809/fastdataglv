...
class TicketsAjaxAPI extends AjaxController {
    ...
    function _search($req) {
        //mail("marco.salmi89@gmail.com","AJAX",$req);
        ...
        //Cliente
        if ($req['clienteId']) {
            $where.=' AND ticket.user_id='.db_input($req['clienteId']);
            $criteria['user_id'] = $req['clienteId'];
        }

        //Help topic
        if($req['utopicId']) {
			if ($req['utopicId']==5935){
            $where.=' AND ticket.user_id=5935 ';
		    }else{
			$where.=' AND ticket.topic_id IN '.str_replace("'","",db_input($req['utopicId'])).' AND ticket.user_id!=5935 ';
			}

            //mail('marco.salmi89@sgmail.com', 'utopic', $where);
            $criteria['utopic_id'] = $req['utopicId'];
        }


        if($req['topicId']&&$req['utopicId']==0) {
        if(!in_array(0,$req['topicId'])) {
			$where.=' AND (';
			foreach ($req['topicId'] as $selectedOption){
            $where.=' ticket.topic_id='.db_input($selectedOption).' OR';
		    }
        ...
        if($req['topicIdo']) {
			$valore=$req['topicIdo'];
            $where.=" AND cdata.group_last_name LIKE '%".db_real_escape($valore)."%' ";
            //$where.=' AND cdata.comm_id=73 ';
            //$criteria['comm_id'] = 73;
        }
        ...
        // Flags
        if ($req['flago']) {
            switch (strtolower($req['flago'])) {
                case 'noswap':
                    $where.=" AND cdata.zz_ci_ptswap='' AND cdata.zz_ci_ptinterv='' ";
                    //$where .= " AND cdata.comm_id=73 ";
        ...
        $startTime_c  =($req['startDate_c'] && (strlen($req['startDate_c'])>=8))?strtotime($req['startDate_c']):0;
        $endTime_c    =($req['endDate_c'] && (strlen($req['endDate_c'])>=8))?strtotime($req['endDate_c']):0;
        ...
        if ($endTime_c)
            // $endTime should be the last second of the day, not the first like $startTime
            $endTime_c += (60 * 60 * 24) - 1;
        if( ($startTime_c && $startTime_c>time()) or ($startTime_c>$endTime_c && $endTime_c>0))
            $startTime_c=$endTime_c=0;
        ...
        if(!$startTime){
        if($startTime_c) {
            $where.=' AND cdata.zz_dt_clmghw>='.$startTime_c.' ';
            $criteria['created__gte'] = $startTime_c;
        }

        if($endTime_c) {
            $where.=' AND cdata.zz_dt_clmghw<='.$endTime_c.' ';
            $criteria['created__lte'] = $startTime_c;
        }
	    }

        if($req['commessa_id']==73)
        $where .= " AND cdata.comm_id=73 ";

        //mail('openunit3@gmail.com', 'zoccali3', $where);
        // Dynamic fields
        $cdata_search = false;
        ...
        if ($cdata_search or $req['flago'] or $req['topicIdo'] or $req['commessa_id'] or $req['startDate_c'] or $req['endDate_c']){
        ...
        /*$sql_log = "INSERT INTO ost_ticket_mylog (id_log,log) VALUES (NULL,'".addslashes($sql)."')";
        db_query($sql_log);*/
    ...
    function search() {
        ...
        $costo=db_query("select sum(costo_int) from ost_ticket__cdata where ticket_id in (".implode(',',$tickets).")");
        $ricavo1=db_fetch_row($ricavo)[0];
        $costo1=db_fetch_row($costo)[0];
        }else{
        $ricavo1=0;
        $costo1=0;
        }
        ...
        if (count($tickets)) {
            ...
            //$result['success'] = print_r($_REQUEST, true);
    ...
    function previewTicket ($tid) {
        ...
        if ($thisstaff->getDeptId()==6 || $thisstaff->getDeptId()==7){
        include STAFFINC_DIR . 'templates/ticket-preview_new.tmpl.php';
	    }else{
		include STAFFINC_DIR . 'templates/ticket-preview.tmpl.php';
		}
    }

    function previewsos ($tid) {
        ...
        include STAFFINC_DIR . 'templates/ticket-sospensione.tmpl.php';
    }
